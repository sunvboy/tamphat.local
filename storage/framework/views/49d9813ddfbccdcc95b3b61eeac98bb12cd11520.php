  <?php if(!empty($productOnes) && count($productOnes) > 0): ?>
  ...
  <table class="table tableProductOne">
      ...
      <tbody>
          <?php $__currentLoopData = $productOnes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          ...
          <tr class="<?php echo e($stock['disabled']); ?>">
              <td class="hideClickShowTwo" style="<?php if (!empty($check) && $check == 'hide') { ?>display: none;<?php } ?> <?php if (!empty($action) && $action == 'update') { ?>display: none;<?php } ?>">
                  <input type="checkbox" name="checkbox[]" value="<?php echo $v->id; ?>" class="checkbox-item-one <?php echo e($stock['disabled']); ?>">
              </td>
  ...
  <div class="mt-5 pb-5 flex justify-center items-center paginationProductOne">
      <?php echo e($productOnes->links()); ?>

  </div>
  <?php endif; ?><?php /**PATH D:\xampp\htdocs\evox.local\resources\views/product/backend/deals/common/product_one.blade.php ENDPATH**/ ?>
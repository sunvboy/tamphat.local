            ...
            <div class="w-[580px] max-w-full bg-[#f4f6f8] p-6 rounded-xl">
                <div class="flex border-b border-[#eee]">
                    <div class="w-1/2 text-center">
                        <a href="{{route('customer.login')}}" class="font-bold uppercase h-[50px] leading-[50px] border-b  float-left w-full border-[#EE0033]">{{trans('index.Login')}}</a>
                    </div>
                ...
                <form action="{{route('customer.login-store')}}" method="POST" id="form-auth">
                    ...
                    <div class="mt-2">
                        ...
                        <input type="text" class="  border w-full h-11 px-3 focus:outline-none focus:ring focus:ring-red-300  hover:outline-none hover:ring hover:ring-red-300  rounded-lg" name="phone" value="{{old('phone')}}" aria-describedby="emailHelp" placeholder="">
                    </div>
                    <div class="mt-5">
                    ...
                    <div class="mt-5 flex justify-center">
                        <button type="submit" class="btn-submit-contact py-4 px-1 md:px-8 rounded-lg block bg-[#EE0033]  text-white transition-all leading-none text-f15 font-bold"> {{trans('index.Login')}}</button>
                    </div>
                    <div class="mt-5 ">
                        @if (config('app.locale') == 'vi')
                        <p class="text-f16 text-center leading-6">
                            <span>Bạn chưa có tài khoản?</span>
                            Nhấn vào đây để <a href="{{route('customer.register')}}" class="text-blue-700 underline">đăng kí</a><br>
                        </p>
                        @else
                        <p class="text-f16 text-center leading-6">

             ...
             <div class="container mx-auto px-3">
                 <div class="flex flex-wrap justify-between mx-[-10px]">
                     <div class="w-full md:w-4/12 px-[10px]">
                         <div class="menu-top">
                             @if($menu_top && count($menu_top->menu_items) > 0)
                             <ul class="flex flex-wrap justify-start space-x-5">
                                 @foreach($menu_top->menu_items as $key=>$item)
                                 <li>
                                     <a href="{{url($item->slug)}}" class="inline-block text-f14 text-white">{{$item->title}}</a>
                                 </li>
                                 @endforeach
                             </ul>
                             @endif
                     ...
                     <div class="w-full md:w-8/12 px-[10px]">
                         <ul class="flex flex-wrap justify-end space-x-10">
                             <li class="text-f14 text-white ">
                                 <i class="fa-solid fa-clock mr-[5px]"></i>{{$fcSystem['contact_time']}}
                             </li>
                             <li class="text-f14 text-white ">
             ...
             <div class="container mx-auto px-3">
                 <div class="flex flex-wrap justify-between items-center mx-[-15px]">
                     <div class="w-1/4 px-[15px]">
                         <form action="{{route('homepage.search')}}" class="flex items-center space-x-3">
                     ...
                     <div class="w-1/4 px-[15px]">
                         <div class="main-logo text-center">
                             <a href="{{getUrlHome()}}">
                                 <img src="{{asset($fcSystem['homepage_logo'])}}" class="inline-block" alt="{{$fcSystem['homepage_company']}}" />
                             </a>
                         </div>
                     </div>
                     <div class="w-1/4 px-[15px]">
                         <div class="flex items-center space-x-5 justify-end">
                             ...
                             @endif
                             <a class="relative" href="{{route('homepage.wishlist_index')}}">
                                 <svg class="w-6" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                             ...
                             <a class="relative tp-cart" href="javascript:void(0)">
                                 ...
                                 <span class="cart-quantity absolute rounded-full -right-1 top-0 text-white text-[0.5625rem] w-3 h-3 bg-black flex items-center justify-center" aria-hidden="true">{{$cart['quantity']}}</span>
                             </a>
                         </div>
                     </div>
                 </div>
                 ...
                 <div class="main-menu flex flex-wrap">
                     <ul class="flex lg:flex-grow md:space-x-0 lg:space-x-4 flex-wrap justify-center">
                         ...
                         <li class="group relative">
                             <a href="{{url($item->slug)}}" class="inline-block px-[10px] lg:px-[25px] uppercase py-[15px] text-f15 transition-all relative hover:opacity-80">

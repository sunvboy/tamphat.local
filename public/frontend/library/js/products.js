...
$(document).on("click", ".js_add_wishlist", function (e) {
    var id = $(this).attr('data-id');
    $.ajax({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
        url: BASE_URL_AJAX + "wishlist",
        type: "POST",
        dataType: "JSON",
        data: {
            id: id,
        ...
        success: function (data) {
            if(data.status == 400){
                ...
                $('.js_add_wishlist_'+id).removeClass('fa-solid');
                $('.js_add_wishlist_'+id).addClass('fa-regular');
                $('.box_wishlist_'+id).remove();
                $('.quantity-wishlist-header').html(data.quantity);
            }else{
                toastr.success(data.message, "Thông báo!");
                $('.js_add_wishlist_'+id).removeClass('fa-regular');
                $('.js_add_wishlist_'+id).addClass('fa-solid');
                $('.quantity-wishlist-header').html(data.quantity);
            }
        },
    });

...
class RoomCategoryController extends Controller
{
    ...
    {
        $module = $this->table;
        $data =  RoomCategory::where('alanguage', config('app.locale'))->with('user')->with('rooms')->orderBy('lft', 'ASC');
        ...
        return view('room.backend.category.index', compact('data', 'module', 'configIs'));
    }
    ...
    public function create()
    {
        ...
        return view('room.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($request->file('image'))) {
            $image_url = uploadImageNone($request->file('image'), 'room-categories');
        } else {
        ...
        return redirect()->route('room_categories.index')->with('success', "Thêm mới danh mục room thành công");
    }

    public function edit($id)
    {
        $module = $this->table;
        $detail  = RoomCategory::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('room_categories.index')->with('error', "Danh mục room không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('room.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('room_categories.index')->with('success', "Cập nhập danh mục room thành công");
    }
    ...
    public function submit($request = [], $action = '', $id = 0, $arrayImg = [])
    {
        ...
        if ($action == 'create') {
            $id = RoomCategory::insertGetId($_data);
        } else {
            RoomCategory::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {

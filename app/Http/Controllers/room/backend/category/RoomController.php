...
namespace App\Http\Controllers\room\backend\category;
...
class RoomController extends Controller
{
    ...
    {
        $this->Nestedsetbie = new Nestedsetbie(array('table' => 'room_categories'));
    ...
    {
        $module = $this->table;
        $data =  Room::where(['alanguage' => config('app.locale')])
            ->with('user:id,name')
            ->with(['catalogues_relationships' => function ($query) {
                $query->select('catalogues_relationships.moduleid', 'room_categories.title', 'room_categories.id')
                    ->where('module', '=', $this->table)
                    ->join('room_categories', 'room_categories.id', '=', 'catalogues_relationships.catalogueid');
            }])
            ->orderBy('order', 'ASC')
            ->orderBy('id', 'DESC');
        ...
        $data =  $data->select('rooms.id', 'rooms.image', 'rooms.title', 'rooms.slug', 'rooms.room_category_id', 'rooms.user_id', 'rooms.created_at', 'rooms.publish', 'rooms.order', 'rooms.ishome', 'rooms.highlight', 'rooms.isaside', 'rooms.isfooter');
        ...
        return view('room.backend.room.index', compact('data', 'module', 'htmlOption', 'configIs'));
    }
    ...
    public function create()
    {
        ...
        return view('room.backend.room.create', compact('module', 'htmlCatalogue',  'dropdown', 'field'));
    }
    public function store(Request $request)
    {
        $request->validate([
            ...
            'room_category_id' => 'required|gt:0',
        ], [
            'title.required' => 'Tiêu đề là trường bắt buộc.',
            'slug.required' => 'Đường dẫn bài viết là trường bắt buộc.',
            'slug.unique' => 'Đường dẫn bài viết đã tồn tại.',
            'room_category_id.gt' => 'Danh mục là trường bắt buộc.',
        ]);
        if (!empty($request->file('image'))) {
            $image_url = uploadImageNone($request->file('image'), 'rooms');
        } else {
            $image_url = $request->image_old;
        }
        $this->submit($request, 'create', 0, $image_url);
        return redirect()->route('rooms.index')->with('success', "Thêm mới room thành công");
    }
    public function edit($id)
    {
        $dropdown = getFunctions();
        $module = $this->table;
        $detail  = room::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('articles.index')->with('error', "room không tồn tại");
        }
        $htmlCatalogue = $this->Nestedsetbie->dropdown([], config('app.locale'));
        $getCatalogue = [];
        if (old('catalogue')) {
        ...
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('room.backend.room.edit', compact('module', 'detail', 'htmlCatalogue', 'dropdown', 'getCatalogue', 'field'));
    }
    public function update(Request $request, $id)
    {
        $request->validate([
            ...
            'room_category_id.gt' => 'Danh mục chính là trường bắt buộc.',
        ]);
        //upload image
        if (!empty($request->file('image'))) {
            $image_url = uploadImage($request->file('image'), 'rooms');
        } else {
            $image_url = $request->image_old;
        }
        //end
        $this->submit($request, 'update', $id, $image_url);
        return redirect()->route('rooms.index')->with('success', "Cập nhập room thành công");
    }
    ...
    public function submit($request = [], $action = '', $id = 0, $image_url = '')
    {
        ...
        $tmp_catalogue[] = (int)$request['room_category_id'];
        if (isset($catalogue)) {
            foreach ($catalogue as $v) {
                if ($v != 0 && $v != $request['room_category_id']) {
                    $tmp_catalogue[] = (int)$v;
                }
            }
        }
        //lấy danh mục cha (nếu có)
        $detail = RoomCategory::select('id', 'title', 'slug', 'lft')->where('id', $request['room_category_id'])->first();
        $breadcrumb = RoomCategory::select('id', 'title')->where('alanguage', config('app.locale'))->where('lft', '<=', $detail->lft)->where('rgt', '>=', $detail->lft)->orderBy('lft', 'ASC')->orderBy('order', 'ASC')->get();
        ...
        if ($action == 'create') {
            $id = Room::insertGetId($_data);
        } else {
            Room::find($id)->update($_data);
        }
        if (!empty($id)) {
            //xóa khi cập nhập
            if ($action == 'update') {
            ...
            $this->Helper->catalogue_relation_ship($id, $request['room_category_id'], $tmp_catalogue, $this->table);
            //thêm router

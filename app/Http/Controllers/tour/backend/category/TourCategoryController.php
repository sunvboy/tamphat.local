...
class TourCategoryController extends Controller
{
    ...
    {
        $module = $this->table;
        $data =  TourCategory::where('alanguage', config('app.locale'))->with('user')->with('tours')->orderBy('lft', 'ASC');
        ...
        return view('tour.backend.category.index', compact('data', 'module', 'configIs'));
    }
    ...
    public function create()
    {
        ...
        return view('tour.backend.category.create', compact('module', 'htmlCatalogue', 'field'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($request->file('image'))) {
            $image_url = uploadImageNone($request->file('image'), 'tour-categories');
        } else {
    ...
    {
        $module = $this->table;
        $detail  = TourCategory::where('alanguage', config('app.locale'))->find($id);
        if (!isset($detail)) {
            return redirect()->route('tour_categories.index')->with('error', "Danh mục tour không tồn tại");
        }
        $htmlCatalogue = $this->nestedsetbie->dropdown([], config('app.locale'));
        $field = \App\Models\ConfigColum::where(['trash' => 0, 'publish' => 0, 'module' => $module])->get();
        return view('tour.backend.category.edit', compact('detail', 'htmlCatalogue', 'module', 'field'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('tour_categories.index')->with('success', "Cập nhập danh mục tour thành công");
    }
